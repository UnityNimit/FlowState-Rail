...
class Optimizer:
    def __init__(self, simulation_instance):
        ...
        # MODIFIED: New priority order with Shatabdi as highest
        self.priorities = {
            'Shatabdi':   10,
            'Rajdhani':   9,
            'Passenger':  8,
    ...
    def generate_plan(self, trains_to_plan, current_state, current_priorities):
        """
        Generates a plan for trains, considering the current state of all active trains on the network.
        """
        ...
        # --- Step 1: Create fixed reservations for RUNNING trains ---
        for train in all_active_trains:
            if train['state'] != 'RUNNING' or not train['route']:
                continue

            print(f"🧠 Optimizer: Accounting for running train {train['id']}.")
        ...
        # --- Step 2: Create decision variables for trains WAITING_PLAN ---
        ...
        # --- Step 3: Add No-Overlap constraints for all resources ---
        for resource_id, intervals in resource_intervals.items():
            if len(intervals) > 1:
                model.AddNoOverlap(intervals)

        # --- Step 4: Define the objective function (Minimize weighted completion time) ---
        ...
        for train in trains_to_plan:
            ...
            if train_end_times:
                ...
                # --- NEW: DYNAMIC PRIORITY LOGIC ---
                priority = 1 # Base priority

                # 1. Punctuality Priority (Highest Override)
                if current_priorities.get('punctuality', False) and train.get('waiting_since') is not None:
                    wait_time = current_state['timestamp'] - train['waiting_since']
                    if wait_time > 600: # 10 minutes
                        priority = 1000 # Massive priority boost to override all others
                        print(f"  -> PUNCTUALITY ALERT: Train {train['id']} waiting for {int(wait_time)}s. Boosting priority.")

                # 2. Train Type Priority (if not overridden by punctuality)
                if priority < 1000 and current_priorities.get('trainType', False):
                    priority = self.priorities.get(train['type'], 1)
       ...
       # --- Step 6: Construct the plan from the solution (No changes here) ---
            ...
            for train in trains_to_plan:
                ...
                for i, choice_var in enumerate(route_choices[train['id']]):
                    if solver.Value(choice_var) == 1:
                        ...
                        print(f"  -> Plan for {train['id']} ({train['type']}, Priority: {self.priorities.get(train['type'], 1)}): {action} on route starting with {first_segment} at {start_time}.")
                        break
            return plan
        return []